...
def initialize_worker():
    """
    Initializes models and MinIO client in the main process.
    ...
    # Initialize the Florence-2 model and move it to the specified device
    ...
    # Ensure that processor outputs are also set to the correct device
    processor = AutoProcessor.from_pretrained(
        "microsoft/Florence-2-large",
        trust_remote_code=True
    )

    # Initialize the zero-shot classification pipeline and specify GPU device
    classifier = pipeline(
    ...
    if obj.object_name.endswith(('.mp4', '.avi', '.mov', '.mkv')) and obj.object_name not in processed_files:
        try:
            ...
            military_keywords = [
                ...
                "military convoy", "camouflage", "tracked vehicle"
            ]
            ...
            with tqdm(total=total_frames, desc=f"Processing {obj.object_name}") as pbar:
                while cap.isOpened():
                    ...
                    if frame_count % frame_interval == 0:
                        if prev_frame is None or cv2.norm(frame, prev_frame, cv2.NORM_L1) > threshold:
                            prev_frame = frame
                            pil_image = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

                            # Process frame and move tensors to GPU
            ...
            most_frequent_category = max(complete_label_counts, key=complete_label_counts.get) if label_counts else 'nonmilitary'

            if nonmilitary_count > (frame_count * 0.5):
            ...
            logging.error(f"Error processing video '{obj.object_name}': {str(e)}", exc_info=True)
    else:
